...
export const RemoteObjectViewer = observer( ({ globalState, clientState, unsafeClient, setGlobalLocation } : 
            ...
            <Stack id="remote-object-viewer-page-layout" direction="row" sx={{ flexGrow: 1, display: 'flex'}}>
                {!unsafeClient? 
                    <div id='tree-view-div'>
                        <CustomizedTreeView 
                            fetchRemoteObjectInfo={clientState.fetchRemoteObjectInfo}
                            setCurrentRemoteObject={clientState.updateURLprefixes}
                            globalState={globalState}
                            sx={{ flexGrow: 0.25, maxWidth: 300, overflowY: 'auto', pr : 3 }} 
                        /> 
                        <Divider id="tree-view-and-client-divider" orientation="vertical" />
                    </div>                        
                    : null 
                }
                <Stack 
                ...
                >
                    ...
                    />
                    <UndockableInteractionWindow clientState={clientState} globalState={globalState} />
                    <ErrorBoundary clientState={clientState} />      
                    <ResponseLogs clientState={clientState} />            
                    <UndockableConsole clientState={clientState} globalState={globalState} /> 
                </Stack>
            </Stack>
        </Box>
    )
...
const UndockableInteractionWindow = observer(( { globalState, clientState } : TabPanelProps ) => {
...
const Functionalities = observer(( { clientState, globalState, name, undocked } : FunctionalitiesProps ) => {
    ...
    switch(name) {
        ...
        case 'Log Viewer' : return <LiveLogViewer clientState={clientState} globalState={globalState }/>
...
export const ObjectSelectWindow = observer(( { clientState, globalState, name, undocked } : FunctionalitiesProps ) => {
...
export const ClientSelect = ({ clientState, globalState, object, name } : ClientSelectProps) => {
...
type UnsafeClientProps = {
    setGlobalLocation : any
}

export const UnsafeClient = (props : UnsafeClientProps) => {
